...
export abstract class Command<Payload, ReturnData = void> {
	...
	async PreRun() {
		e.RemoveHelpers(this.payload); // have this run locally, before sending, to save on bandwidth
		this.Validate_Early();
		await this.Prepare();
		await this.Validate();
	}
	...
	async Run(maxUpdatesPerChunk = e.maxDBUpdatesPerBatch): Promise<ReturnData> {
		if (commandsWaitingToComplete.length > 0) {
			e.MaybeLog_Base(a=>a.commands, l=>l(`Queing command, since ${commandsWaitingToComplete.length} ${commandsWaitingToComplete.length == 1 ? "is" : "are"} already waiting for completion.${""
		...
		try {
			...
			await e.ApplyDBUpdates(e.DBPath(), dbUpdates);
