...
export function DoesActionSetFirestoreData(action: Action<any>) {
	// the LISTENER_RESPONSE and DOCUMENT_ADDED actions dont always actually change the store data (sometimes it stores the same data, meaning it only causes a meaningless status-update: {receiving:false, received:true})
