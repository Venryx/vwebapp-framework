...
export function Watch<T>(accessor: (watcher: Watcher)=>T, dependencies: any[]): T {
	...
	const proceed = ()=>{
		// if mobx-compat-mode enabled, remove dependency-changed checking (otherwise the MobX accesses don't always go through, breaking its dependency tracking)
		if (manager.mobxCompatMode || watcher.needsRerun || !shallowEqual(dependencies, watcher.lastDependencies)) {
